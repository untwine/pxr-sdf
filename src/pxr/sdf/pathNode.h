...
inline void TfDelegatedCountIncrement(const SDF_NS::Sdf_PathNode* p) noexcept {
    p->_refCount.fetch_add(1, std::memory_order_relaxed);
}
inline void TfDelegatedCountDecrement(const SDF_NS::Sdf_PathNode* p) noexcept {
    if ((p->_refCount.fetch_sub(1) & SDF_NS::Sdf_PathNode::RefCountMask) == 1) {
        p->_Destroy();
    }
}
